...
## Sprint 5(番外編): Laravel Sail への移行

**環境の載せ替えでもアプリのコードは 1 行も変わらない**ことを体感できるスプリント。
仕様変更の経緯は [qa-log.md](qa-log.md) の Q7。

### Step 15: Sail 導入 — `72b0296`
[GitHub](https://github.com/morisaki-yuichi/todo-app-example1/commit/72b0296) / `git show 72b0296`

- **見るファイル**: `src/compose.yaml`(sail:install による自動生成)
- **ポイント**:
  - 旧構成(`git show 884b072:docker-compose.yml`)と**並べて見比べる**のがおすすめ。
    値の直書き → `${APP_PORT:-80}` のような **.env 参照**に変わっている
  - イメージの pull ではなく `vendor/laravel/sail/runtimes/` の Dockerfile から**ローカルビルド**
  - ファイル名が `docker-compose.yml` ではなく `compose.yaml`(Docker Compose の現行標準名)
  - `composer.json` の `require-dev` に注目 — Sail は「開発時だけの依存」

### Step 16: 旧構成の削除 — `697d69c`
[GitHub](https://github.com/morisaki-yuichi/todo-app-example1/commit/697d69c)

- ルートの `docker-compose.yml` を削除。**削除もコミットとして履歴に残る**ので、
  bitnami 時代の構成はいつでも `git show 884b072:docker-compose.yml` で参照できる

### Sprint 5 の実録トラブル(レビュー記録参照)

- Vite ポート 5173 の衝突 → Sprint 1 と同じパターンだが、今回は **.env に 1 行足すだけ**で解決
- 移行直後の 500(`sessions テーブルがない`)→ bitnami が自動でやっていたマイグレーションを
  Sail では自分で叩く、という差異の実例
- 詳細: [Sprint 5 レビュー](../05_sprint5/sprint-review.md)
